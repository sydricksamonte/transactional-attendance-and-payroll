...
#######SYD
#$curr_date = mktime(0,0,0,$s_month,$s_day,date("Y"));
#$yearend_date = mktime(23,59,59,$e_month,$e_day,date("Y"));
...
?>
<?php 
$basic =($employee['Employee']['monthly']);
$d_rate = $basic / 22;
$h_rate = $d_rate / 8;
$m_rate = $h_rate / 60;
...
<?php $otamount = formatAmount($ot1_amount + $ot2_amount + $ot3_amount + $ot4_amount + $ot5_amount);  ?>
...
<table border=1>
  ...
  <tr>
    ...
    <td>#123</td>
    <td><?=formatAmount($basic/2)?></td>
    <td><?=$monthStart.'-'.$dayCutVal?></td>
    <td><?=formatAmount($d_rate)?></td>
    <td><?=formatAmount($h_rate)?></td>
    <td><?=formatAmount($m_rate)?></td>
    <td><?=$absent_total?></td>
    <td><?=$late_total?></td>
    <td><?=formatAmount($deduction_amount)?></td>
    <td><?=formatAmount($otamount)?></td>
    <td><?=formatAmount($ndamount)?></td>
    <td><?=formatAmount($hdamount)?></td>
    <td><?=formatAmount($net_pay)?></td>
  </tr>
</table>
</div>
